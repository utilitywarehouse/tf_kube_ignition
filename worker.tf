...
data "template_file" "worker-kubelet" {
  ...
  vars = {
    ...
    role              = var.worker_groups[count.index]["role"]
    taints            = var.worker_groups[count.index]["taints"]
  }
}
...
data "ignition_systemd_unit" "worker-kubelet" {
  ...
  content = data.template_file.worker-kubelet[count.index].rendered
}
...
data "ignition_config" "worker" {
  ...
  files = concat(
    ...
    var.worker_groups[count.index]["additional_files"],
  )
  ...
  systemd = concat(
    [
      ...
      data.ignition_systemd_unit.worker-kubelet[count.index].id,
    ...
    var.worker_groups[count.index]["additional_systemd_units"],
  )
}
